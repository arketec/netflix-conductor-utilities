...
export type WorkFunction<OUTPUT = void, INPUT = any> = (
  input: INPUT,
...
class ConductorWorker<OUTPUT = void, INPUT = any> extends EventEmitter {
  ...
  pollAndWork(taskType: string, fn: WorkFunction<OUTPUT, INPUT>) {
    // keep 'function'
    return (async () => {
      // Poll for Worker task
      debug(`Poll a "${taskType}" task`);
      const { data: pullTask } = await this.client.get<PollTask | void>(
      ...
      if (this.needAckTask) {
        ...
        await this.client.post<boolean>(
      ...
      const runningTask: ProcessingTask<OUTPUT> = {
        taskId,
        task: new RunningTask<OUTPUT>(this, {
          ...baseTaskInfo,
          ...this.runningTaskOptions
        })
      };
      this.runningTasks.push(runningTask as any);
