...
export default class RunningTask<Result = void> {
  ...
  constructor(
    ...
    } = options?.keepAliveTimer || {};
    this.options = {
      ...options,
      keepAliveTimer: { enable, interval, callbackAfterSeconds }
    };
    this.done = false;
  }

  updateTaskInfo(
    partialUpdateTaskInfo: Partial<UpdatingTaskResult> & { status: TaskState }
  ...
  private __setKeepTaskTimerForNotifyConductor() {
    ...
    debug(
      `start a keeping-task timer: ${this.options.keepAliveTimer.interval}`
    );
    this.keepRunningTimer = setInterval(() => {
      const callbackAfterSeconds =
        this.options.keepAliveTimer.callbackAfterSeconds;
  ...
  startTask() {
    ...
    this.options.keepAliveTimer.enable &&
  ...
  async sendLog(msg: string, others: Partial<UpdatingTaskResult> = {}) {
    ...
    if (this.options.keepAliveTimer.enable) {
      otherInfo.callbackAfterSeconds =
        this.options.keepAliveTimer.callbackAfterSeconds;
    }
